...
class PaymentGatewayPacketsRestLogicBo extends PaymentGatewayPacketsRestLogic {
  ...
  public function start($id, $business_unit, $product_type, $params, $request, $target_msisdn) {
    ...
    $this->transactions->addValueToAdditionalData('offer', $offer);
    $fields = [
  ...
  public function generateOrderId($business_unit, $product_type, $id_type, $id, $purchaseorder_id) {
    ...
    $this->addAutoPacketsAdditionalData($data_transaction);
    $this->transactions->updateDataTransactionOrderInProgress($decrypt_purchaseorder_id, $order, $this->transactions->getAdditionalData());
    $body_logs = $body;
   ...
   * {@inheritdoc}
   */
  protected function getOfferAmountLessClick($offer, $tax) {
    return $offer['cost'][0]['amount'];
  }

  /**
   * {@inheritdoc}
   */
  protected function getOfferIdLessClick($offer, $params) {
