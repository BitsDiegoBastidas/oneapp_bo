...
class EnrollmentsServiceBo extends EnrollmentsService {
  ...
  public function getSubscriptionDataOfAdditionalData($id, $addtional_data) {
    $offer = $this->getValueOfAdditionalData('offer');
    $billing_data = $this->getValueOfAdditionalData('billingData');
    $hours = $this->getSuscriptionDurationInHours($offer);
    $this->params['subscription'] = [
      'name' => $offer->name,
      'amount' => (string) $offer->cost,
      'duration' => $this->getDurationRenewOffer($hours),
      'productReference' => (string) $offer->offerId,
      'lastOrderTimeStamp' => str_replace(" ", "T", date_format(date_create(), 'Y-m-d H:i:sP')),
    ];
    $this->params['billingData'] = $billing_data;
  }
  ...
  public function getTokenizedCardOfDatabase($addtional_data) {
    if (empty($this->params["tokenizedCardId"])) {
      $payment_token_id = $this->getValueOfAdditionalData('paymentTokenId');
      if (!empty($payment_token_id)) {
        $this->params['tokenizedCardId'] = $payment_token_id;
      }
      $number_card = $this->getValueOfAdditionalData('numberCard');
      if (!empty($number_card)) {
        $this->params['tokenizedCardId'] = isset($this->params['tokenizedCardId']) ?
          $this->params['tokenizedCardId'] : $this->getPaymentTokenId($id, $number_card);
      }
    }
  }
  ...
  public function getParamsSubscribers($id) {
    ...
    $email_transaction = $this->getValueOfAdditionalData('email');
    $transaction_billing_data = $this->getValueOfAdditionalData('billingData');
    $email_transaction = (empty($email_transaction) && isset($transaction_billing_data["email"])) ? $transaction_billing_data["email"] : $email_transaction;
    ...
    $this->getTokenizedCardOfDatabase($this->transaction->additionalData);
    if (!empty($this->transaction->additionalData)) {
      $this->getSubscriptionDataOfAdditionalData($id, $this->transaction->additionalData);
    }
