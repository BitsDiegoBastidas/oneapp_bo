          ...
          if (isset($config["general"]["label_balances"]["show"]) && $config["general"]["label_balances"]["show"] && !empty($wallet) && !in_array($balance->wallet, $wallet)) {
            ...
            foreach ($configHeaders as $field_name => $field) {
              switch ($field_name) {
                ...
                case 'endDateTime':
                  $balance->expirationDate = empty($balance->expirationDate) ? '' : $balance->expirationDate;
                  $end_date_time = !empty($balance->expirationDate) ? new \DateTime($balance->expirationDate) : NULL;
                  $date_now = new \DateTime('now');
                  $diff = !empty($end_date_time) ? $end_date_time->diff($date_now) : NULL;
                  $show = !empty($balance->expirationDate) ? (bool) $configHeaders[$field_name]['show'] : FALSE;
