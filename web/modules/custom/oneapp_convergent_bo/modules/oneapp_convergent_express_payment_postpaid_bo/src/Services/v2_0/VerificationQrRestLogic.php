...
class VerificationQrRestLogic {
    ...
    try {
      ...
      foreach ($this->configBlock["fields"] as $key => $field) {
        switch ($key) {
            ...
            break;
            case "paymentReceipt":
                $row[$key]['details'] = $this->parsePaymentReceipt($id, $field);
            break;
            ...
            break;
            case "labelPaymentReceipt":
                $row['paymentReceipt']['label'] = $field['label'];
                $row['paymentReceipt']['show'] = (bool) $field['show'];
            break;
            default:
            break;
        }
  ...
  public function getConfig() {
    ...
    $this->buildGetInputsForm($config);
  ...
  public function parsePaymentDetails($id, $fields){
    return [
        "billPayment" => [
            "label" => $fields["billPayment"]["label"],
            "value" => "",
            "show" => (bool) $fields["billPayment"]["show"]
        ],
        "period" => [
  ...
  public function parsePaymentMethods($id, $fields){
    return [
        "methods" => [
            "label" => $fields["methods"]["title"],
            "value" => $fields["methods"]["label"],
  ...
  /**
   * Build get invoices form structure.
   *
   * @param array $config
   *   Array where configuration structure is stored.
   */
  protected function buildGetInputsForm(array &$config) {
    $config['form'] = [];
    $size_error_validation = [
      '#size' => 40,
    ];
    foreach ($this->configBlock['config']['form'] as $id => $entity) {
      $item = [];

      $item['label'] = $entity['label'];
      $item['value'] = $entity['value'];
      $item['placeholder'] = $entity['placeholder'];
      $item['validations'] = ['required' => (bool) $entity['required'], 'minLength' => $entity['minLength'], 'maxLength' => $entity['maxLength'], 'pattern' => $entity['pattern']];
      $item['description'] = $entity['description'];
