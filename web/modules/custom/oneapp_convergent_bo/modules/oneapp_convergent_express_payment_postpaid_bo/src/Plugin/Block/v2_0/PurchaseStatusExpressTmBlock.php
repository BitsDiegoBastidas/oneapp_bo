...
/**
 * Provides a 'GeneratePurchaseOrdersBlock' block.
 *
 * @Block(
 *  id = "oneapp_convergent_express_payment_postpaid_tm_v2_0_purchase_status_block",
 *  admin_label = @Translation("OneApp Convergent Express Payment TigoMoney Purchase Status v2.0 - (Estado de transacción)"),
 *  group = "oneapp_convergent_payment_gateway_tm"
 * )
 */
class PurchaseStatusExpressTmBlock extends BlockBase {
  ...
  public function defaultConfiguration() {
    ...
    $this->messages = [
      ...
      'order_in_progress' => [
        'title' => '¡TigoMoney pendiente de pago!',
        'body' => 'Ingresa a la app de tu banco para realizar el pago y aguarda el correo de confirmación',
      ],
  ...
  public function adfBlockForm($form, FormStateInterface $form_state) {
    ...
    foreach ($fields as $id => $entity) {
      ...
      $description='Para configurar el label para home y mobile se debe separar por | ';
      $description=$description.'dejando primero configurado home y luego mobile así: labelhome|labelmobile';
    ...
    for ($i = 0; $i < $count; $i++) {
      ...
      $aditional_id = "data{$i}";
      $var_aux=$this->configuration["aditional"][$aditional_id]["label"];
      $form['aditional']['fields'][$aditional_id]['label'] = [
        '#type' => 'textfield',
        '#default_value' => isset($var_aux) ? $this->configuration["aditional"][$aditional_id]["label"] : '',
      ];
      $var_aux=$this->configuration["aditional"][$aditional_id]["variable"];
      $form['aditional']['fields'][$aditional_id]['variable'] = [
        '#type' => 'textfield',
        '#default_value' => isset($var_aux) ? $this->configuration["aditional"][$aditional_id]["variable"] : '',
      ];
      $var_aux=$this->configuration["aditional"][$aditional_id]["show"];
      $form['aditional']['fields'][$aditional_id]['show'] = [
        '#type' => 'checkbox',
        '#default_value' => isset($var_aux) ? $this->configuration["aditional"][$aditional_id]["show"] : '',
      ];
    ...
    $var_aux=$this->configuration['messages']['fulfillment_complete']['title'];
    $form['messages']['fulfillment_complete']['title'] = [
      '#type' => 'textfield',
      '#default_value' => isset($var_aux) ? $var_aux : $this->messages['fulfillment_complete']['title'],
    ];
    $var_aux=$this->configuration['messages']['fulfillment_complete']['body'];
    $form['messages']['fulfillment_complete']['body'] = [
      '#type' => 'textarea',
      '#default_value' => isset($var_aux) ? $var_aux : $this->messages['fulfillment_complete']['body'],
    ];
    ...
    $var_aux=$this->configuration['messages']['order_in_progress']['title'];
    $form['messages']['order_in_progress']['title'] = [
      '#type' => 'textfield',
      '#default_value' => isset($var_aux) ? $var_aux : $this->messages['order_in_progress']['title'],
    ];
    $var_aux=$this->configuration['messages']['order_in_progress']['body'];
    $form['messages']['order_in_progress']['body'] = [
      '#type' => 'textarea',
      '#default_value' => isset($var_aux) ? $var_aux : $this->messages['order_in_progress']['body'],
    ];
    ...
    $var_aux=$this->configuration['messages']['payment_non_complete']['title'];
    $form['messages']['payment_non_complete']['title'] = [
      '#type' => 'textfield',
      '#default_value' => isset($var_aux) ? $var_aux : $this->messages['payment_non_complete']['title'],
    ];
    $var_aux=$this->configuration['messages']['payment_non_complete']['body'];
    $form['messages']['payment_non_complete']['body'] = [
      '#type' => 'textarea',
      '#default_value' => isset($var_aux) ? $var_aux : $this->messages['payment_non_complete']['body'],
    ];
    ...
    $var_aux=$this->configuration['messages']['fulfillment_non_complete']['title'];
    $form['messages']['fulfillment_non_complete']['title'] = [
      '#type' => 'textfield',
      '#default_value' => isset($var_aux) ? $var_aux : $this->messages['fulfillment_non_complete']['title'],
    ];
    $var_aux=$this->configuration['messages']['fulfillment_non_complete']['body'];
    $form['messages']['fulfillment_non_complete']['body'] = [
      '#type' => 'textarea',
      '#default_value' => isset($var_aux) ? $var_aux : $this->messages['fulfillment_non_complete']['body'],
    ];
    $var_aux=$this->configuration['messages']['payment_non_complete']['codeMapping'];
    $form['messages']['payment_non_complete']['codeMapping'] = [
      '#title' => t('Mapeo de codigos de errores recibios en callbacks'),
      '#type' => 'textarea',
      '#default_value' => isset($var_aux) ? $var_aux : '',
      '#description' => t('Mapeo de mensajes para codigos de error recibidos en callbacks e.g: 404|Mensaje personalizado'),
    ];
    $var_aux=$this->configuration['messages']['fulfillment_non_complete']['codeMapping'];
    ...
    $var_aux=$this->configuration['errorApi']['error_default'];
    $form['errorApi']['error_default'] = [
      '#type' => 'textfield',
      '#title' => $this->t("Mensaje de error por defecto"),
      '#default_value' => isset($var_aux) ? $var_aux : '',
    ];
    $var_aux=$this->configuration['errorApi']['error_mapping'];
