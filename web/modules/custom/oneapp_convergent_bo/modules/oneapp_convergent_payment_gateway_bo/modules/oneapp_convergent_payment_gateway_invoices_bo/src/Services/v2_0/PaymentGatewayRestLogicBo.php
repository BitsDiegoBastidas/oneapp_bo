...
class PaymentGatewayRestLogicBo extends PaymentGatewayRestLogic {
  ...
  public function start($id, $id_type, $business_unit, $product_type, $params) {
    ...
    $fields['additionalData'] = $this->getSerializedAdditionalData($balance, $account_number_invoice_payment, $params);
  ...
  public function generateOrderId($business_unit, $product_type, $id_type, $id, $purchaseorder_id) {
    ...
    $this->unserializeAdditionalData($data_transaction);
    $this->transactions->addValueToAdditionalData('enrollMe', $this->params['enrollMe'] ?? FALSE);
    $is_multipay = $this->getValueOfAdditionalData('isMultipay');
    $fields_for_payment_body = $this->getValueOfAdditionalData('fieldsForPaymentBody');
    $is_multipay = (!empty($is_multipay)) ? TRUE : FALSE;
    $additional_data_for_payment_body = (!empty($fields_for_payment_body)) ? $fields_for_payment_body : [];
    if ($is_multipay) {
    ...
    $fields = [
      ...
      'additionalData' => $this->transactions->serializeAdditionalData(),
    ];
