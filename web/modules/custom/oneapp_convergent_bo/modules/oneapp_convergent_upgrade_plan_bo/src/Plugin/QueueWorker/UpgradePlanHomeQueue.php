...
final class UpgradePlanHomeQueue extends QueueWorkerBase implements ContainerFactoryPluginInterface {
  ...
  */
  public function processItem($data) {
    ...
    $max_retries = isset($config_block['async_config']['cron']['max_retries'])
      ? $config_block['async_config']['cron']['max_retries']
      : 3;
    $wo_upgrade_window = isset($config_block['async_config']['general']['wo_upgrade_window'])
      ? $config_block['async_config']['general']['wo_upgrade_window']
      : 120;
    $pending_upgrade_retry = $service->processPendingUpgrade($pending_upgrade, (int)$max_retries, (int)$wo_upgrade_window);
    ...
    if ($pending_upgrade_retry) { // if need to retry, add new item to be processed the queue in the future
      ...
      $data->time = \Drupal::time()->getCurrentTime() + 35;
