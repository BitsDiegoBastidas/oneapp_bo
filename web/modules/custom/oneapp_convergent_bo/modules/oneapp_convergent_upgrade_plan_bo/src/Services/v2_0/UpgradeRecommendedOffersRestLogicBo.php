...
class UpgradeRecommendedOffersRestLogicBo extends UpgradeRecommendedOffersRestLogic {
  ...
  public function buildPlanList($current_plans, $recommended_plans) {
    ...
    foreach ($recommended_plans as $rpk => $recommended_plan) {
      ...
      // Plan info
      $plan_list_item = [];
      $plan_list_item['featured'] = FALSE;
      $plan_list_item['planId'] = $recommended_plan['id'];
      $plan_list_item['planType'] = $recommended_plan['type'];
      $plan_list_item['planName'] = [
      ...
      $plan_list_item['products']['offersList'] = [];
      if (isset($current_offers['DECODER']['value'])) {
        $recommended_plan_decoder_count = $recommended_offers['DECODER']['value'] ?? 0;
        if ($recommended_plan_decoder_count < $current_offers['DECODER']['value']) {
          continue;
        }
      }
      $plan_list[$rpk] = $plan_list_item;
  ...
  public function getVerificationPlan($billing_id, $recommended_offers_verification = []) {
    ...
    return [
      ...
      'plans' => [
        'label' => (!empty($config_verification['plans']['label'])) ? $config_verification['plans']['label'] : '',
        'url' => (!empty($config_verification['plans']['url'])) ? $config_verification['plans']['url'] : '#',
        'value' => (!empty($config_verification['plansDesc']['value'])) ? $config_verification['plansDesc']['value'] : '',
        'show' => (!empty($config_verification['plans']['show'])) ? TRUE : FALSE,
      ],
      'additionalText' => [
        'value' => (!empty($config_verification['additionalText']['value'])) ? $config_verification['additionalText']['value'] : '',
        'show' => (!empty($config_verification['additionalText']['show'])) ? TRUE : FALSE,
      ]
    ];
      ...
      foreach ($plan->bundledItemsList as $bundle_key => $offer) {
        ...
        if (stripos($_offer['name'], 'canal') !== FALSE
          ...
          foreach ($channels as $key => $value) {
            ...
            if (isset($result[$plan_id]['offers'][$_offer_key]['value'])) {
              $result[$plan_id]['offers'][$_offer_key]['value'] += $_offer['value'] ?? 0;
            }
            else {
  ...
  public function currentPlanAnalyzer($current_plans) {
    ...
    foreach ($current_plans ?? [] as $plan) {
      ...
      foreach ($plan->productList ?? [] as $product) {
        ...
        $device_list = [];
        if ($product->productType != 'INTERNET') {
          $device_list = $product->deviceList ?? [];
        }
        $offers_and_devices = array_merge($product->offeringList ?? [], $device_list);
        ...
        foreach ($offers_and_devices ?? [] as $offer) {
          ...
          $_current_plans[$_plan_id]['decoders'] =
            $this->currentDecoders[$_plan_id] = max(
              $decoders_by_plan,
              $decoders_by_offer_name,
              $decoders_by_offer_type,
              $has_tv_product
            );
          $_current_plans[$_plan_id]['offers'][$_offer_key] = $_offer;
        }
  ...
  public function getForms() {
    ...
    if (!empty(\Drupal::request()->get('id'))) {
      $response = $this->service->getBeneficiary(\Drupal::request()->get('id'));
