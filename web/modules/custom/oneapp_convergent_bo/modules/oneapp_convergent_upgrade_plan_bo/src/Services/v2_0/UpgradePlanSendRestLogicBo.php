...
class UpgradePlanSendRestLogicBo extends UpgradePlanSendRestLogic {
  ...
  public function isOwnershipValid($id, $data, $id_type) {
    try {
      ...
      if (
        $document_type == $data['confirmIdentity']['identificationType']
  ...
  public function updateCurrentPlanHome($id, $data, $id_type) {
    ...
    $log_id = $this->addLog($fields_to_log);

    if ($log_id
      && $upgrade_status == UpgradePlanLogBoModelService::UPGRADE_STATUS_DONE
      && $order_status == UpgradePlanLogBoModelService::ORDER_STATUS_NOT_GENERATED) {
      $this->enqueue((int)$log_id);
    }
    $email_data = [
      'new_plan'     => $fields_to_log['name_plan'],
      'current_plan' => $fields_to_log['current_plan_name'],
  ...
  public function enqueue(int $upgrade_id) {
    ...
    $data = (object)[
      ...
      "time" => \Drupal::time()->getCurrentTime() + 35,
    ];
   ...
   * @param string $id
   *  The account Id
   * @param int $max_seconds_diff
   *  The maximum number of seconds between the upgrade and the last appointment scheduled
   * @param string $upgrade_date
   *
   * @return object
   *  The last appointment scheduled for the provided account Id
   */
  public function getLastAppointmentScheduledById($id, $max_seconds_diff = null, $upgrade_date = null) {
    ...
    if ($max_seconds_diff && $upgrade_date && is_numeric($max_seconds_diff) && isset($last_appointment->createdAt)) {
      $upgrade_date = \Drupal\Core\Datetime\DrupalDateTime::createFromFormat('Y-m-d H:i:s', $upgrade_date);
      $last_appointment_date = \Drupal\Core\Datetime\DrupalDateTime::createFromFormat('Y-m-d\TH:i:s', $last_appointment->createdAt);
      $diff = abs($upgrade_date->getTimestamp() - $last_appointment_date->getTimestamp());
      if ($max_seconds_diff < $diff) {
        $last_appointment = NULL;
      }
    }
  ...
  public function processPendingScheduleUpgrades() {
    ...
    if (isset($this->configBlock['async_config']['cron'])) {
      ...
      $cron_config = [
        ...
        'wo_upgrade_window'             => $this->configBlock['async_config']['general']['wo_upgrade_window'],
      ];
    }
    ...
    foreach ($pending_upgrades as $pending_upgrade) {
      $pending_upgrade_retry = $this->
        processPendingUpgrade($pending_upgrade, (int)$cron_config['max_retries'], (int)$cron_config['wo_upgrade_window']);
      if ($pending_upgrade_retry) { // if wasn't processed, add it to the queue again
   ...
   */
  public function processPendingUpgrade(Object $pending_upgrade, int $max_retries = 3, int $wo_upgrade_window = 120) {
    $log_data = [];

    // if we didn't have a successful upgrade or the working order is not required, abort
    if ($pending_upgrade->upgrade_status == UpgradePlanLogBoModelService::UPGRADE_STATUS_PENDING
        || $pending_upgrade->order_status != UpgradePlanLogBoModelService::ORDER_STATUS_NOT_GENERATED) {
          return false;
    }
    ...
    $zendesk_data = [
      ...
      'current_plan_name'     => $pending_upgrade->name_plan,
      'recommended_plan_name' => $pending_upgrade->current_plan_name,
      'email'                 => $pending_upgrade->client_email,
    ];

    $last_appointment = $this->
      getLastAppointmentScheduledById($pending_upgrade->billing_account, $wo_upgrade_window, $pending_upgrade->date);
    if (!$last_appointment) {
      $retry = true;
    ...
    if (isset($last_appointment->id)) {
      $pending_upgrade->order_id = $last_appointment->id;
    }
    ...
    if (
      is_null($available_dates) || !is_object($available_dates)
      || (isset($available_dates->error) && $available_dates->error)
      || !isset($available_dates->availableTimeslots)
      || (isset($available_dates->availableTimeslots) && empty($available_dates->availableTimeslots))
    ) {
    ...
    if (
      is_null($scheduled) || !is_object($scheduled)
    ...
    if (isset($scheduled->id)) {
      ...
      if (isset($this->configBlock['emailSetting']['config']['dateFormat'])) {
        $date_format = $this->configBlock['emailSetting']['config']['dateFormat'];
      }
    /** @var \Drupal\Core\Datetime\DateFormatter $date_formatter */
      $date_formatter = \Drupal::service('date.formatter');
      if (isset($scheduled->dates['startDateTime'])) {
        $time_zone = (new \DateTime($scheduled->dates['startDateTime']))->getTimezone()->getName();
        $pending_upgrade->date_visit = $date_formatter
          ->format(strtotime($scheduled->dates['startDateTime']), 'custom', 'Y-m-d H:i:s', $time_zone);
        $log_data['order_scheduled'] = 'Orden agendada: ' . $pending_upgrade->date_visit . " | " . $date;
        $email_data['date_start'] = $date_formatter
          ->format(strtotime($scheduled->dates['startDateTime']), $date_format, '', $time_zone);
      }
      if (isset($scheduled->dates["endDateTime"])) {
        $time_zone = (new \DateTime($scheduled->dates['endDateTime']))->getTimezone()->getName();
        $email_data['date_end'] = $date_formatter
          ->format(strtotime($scheduled->dates['endDateTime']), $date_format, '', $time_zone);
      }
    }
