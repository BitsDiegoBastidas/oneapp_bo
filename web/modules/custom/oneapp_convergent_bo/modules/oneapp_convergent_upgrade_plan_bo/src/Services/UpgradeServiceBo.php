...
class UpgradeServiceBo extends UpgradeService {
  ...
  public function sendEmailHome($data = [], $type = null, $email_to = null) {
    ...
    if (!empty($email_to)) {
      ...
      // validate if email sending is disabled
      if ((isset($email_setting['config']['enableEmailSend']) && !$email_setting['config']['enableEmailSend'])
          // or if email sending by type is disabled
          || (isset($email_setting[$type]['enableSend']) && !$email_setting[$type]['enableSend'])) {
          // go back without sending email
        return;
      }
