    ...
    if (isset($firstPlan->productList) && !empty($firstPlan->productList)) {
      foreach ($firstPlan->productList as $key => $value) {
        if (isset($value->productName) && $value->productName != NULL) {
          $offering_list = [];
          if (isset($value->offeringList) && !empty($value->offeringList)) {
            foreach ($value->offeringList as $key => $offering) {
              $offering_list[$key]['offeringName'] = $offering->offeringName ?? '';
              $offering_list[$key]['offeringType'] = isset($offering->offeringType) ? $offering->offeringType : '';
              $offering_list[$key]['subscriptionNumber'] = $offering->subscriptionNumber ?? '';
              if (!empty($offering->detailList)) {
                foreach ($offering->detailList as $device) {
                  ...
                  $offering_list[$key]['deviceList'][$device->deviceId] = [
          ...
          if (empty($offering_list)) {
            $offering_list[] = [
              'productId' => $value->productId ?? '',
              'offeringName' => $value->productName ?? '',
          ...
          foreach ($offering as $offeringIndex => $offeringValue) {
            if ($offeringField == $offeringIndex) {
              ...
              if ($offeringField == 'offeringName') {
                $portfolio[$productKey]['offeringList'][$key][$offeringField]['label'] = $offeringFieldValue["label"] ?? '';
              }
            }
          }
        }
